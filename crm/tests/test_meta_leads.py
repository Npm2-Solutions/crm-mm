...
class TestMetaCatchUp(IntegrationTestCase):
	"""The short pass that exists only while the webhook is silent."""

	def tearDown(self):
		frappe.db.rollback()
		frappe.clear_cache(doctype="CRM Meta Settings")

	def _last_call(self, when):
		frappe.db.set_single_value("CRM Meta Settings", "last_webhook_seen", when)
		frappe.clear_cache(doctype="CRM Meta Settings")

	def test_it_polls_while_meta_stays_silent(self):
		"""An app in Development mode never receives a leadgen notification, so
		polling is the only way a lead reaches anybody."""
		from crm.integrations.meta import leads as L

		make_form(form_id="990800", page_id="880800")
		frappe.db.set_value("Facebook Page", "880800", "sync_enabled", 1)
		self._last_call(None)

		with patch.object(L, "reconcile_synced_pages") as reconcile:
			L.catch_up_recent_leads()
		reconcile.assert_called_once()

	def test_it_stands_down_once_the_webhook_works(self):
		"""Polling on top of a working webhook is calls spent on nothing."""
		from crm.integrations.meta import leads as L

		make_form(form_id="990801", page_id="880801")
		frappe.db.set_value("Facebook Page", "880801", "sync_enabled", 1)
		self._last_call(frappe.utils.now())

		with patch.object(L, "reconcile_synced_pages") as reconcile:
			L.catch_up_recent_leads()
		reconcile.assert_not_called()

	def test_nothing_switched_on_costs_nothing(self):
		from crm.integrations.meta import leads as L

		frappe.db.sql("update `tabFacebook Page` set sync_enabled = 0")
		with patch.object(L, "reconcile_synced_pages") as reconcile:
			L.catch_up_recent_leads()
		reconcile.assert_not_called()
